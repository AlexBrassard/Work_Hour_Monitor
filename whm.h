...
typedef struct whm_config_type {
  ...
  int                      time_n_half_after_40; /* > 0: Yes; 0: Nope. */
  int                      double_time_after_50; /* > 0: Yes; 0: Nope. */
...
typedef struct whm_sheet_type {
  ...
  struct whm_time_type     *time_o;              /* When a sheet needs a customized time object. */
...
typedef struct whm_week_type {
  ...
  double                   *pos_ot_total_hours;  /* Total overtime hours worked this week, per positions. */
  double                   *pos_ot_total_earnings; /* Total overtime earnings this week, per positions. */
...
typedef struct whm_day_type {
  ...
  double                   *pos_ot_hours;        /* Overtime worked hours for each positions. */
  double                   *pos_ot_earnings;     /* Overtime earnings for each positions. */
...
whm_time_T*    whm_copy_time       (whm_time_T *dest,       /* Copy the time object src into the time object dest. */
				    whm_time_T *src);
...
int            whm_new_year_dir    (whm_config_T *config,  /* Verify and create if needed each company's current year dir. */
				    whm_time_T *time_o);
char*          whm_make_year_path  (whm_config_T *config,  /* Make a pathname given the company and year. */
				    whm_time_T *time_o,
				    char *path);
...
void            whm_reset_totals     (whm_sheet_T *sheet,  /* Reset all sheet totals to -1.0 .                             */
				      whm_config_T *config);
void            whm_get_sheet_overtime(whm_sheet_T *sheet, /* Calculate weekly overtime. */
				       whm_config_T *config);
...
int             whm_complete_week    (whm_sheet_T *sheet, /* Returns the number of hours worked for last week of prev month. */
				      whm_config_T *config,
				      whm_time_T *time_o);
int             whm_get_numof_pos_worked(whm_sheet_T *sheet, /* Gives the number of position worked for a given day. */
					 whm_config_T *config,
					 int week_ind,
					 int day_ind);
int             whm_calculate_overtime(whm_sheet_T *sheet, /* Does exactly what the names implies. */
				       whm_config_T *config,
				       int ot_type,
				       int week_ind,
				       int last_worked_day);
int             whm_last_worked_day  (whm_sheet_T *sheet,  /* Returns the index of the last worked day. */
				      whm_config_T *config,
				      int week_ind,
				      int day_ind);
...
void            whm_print_all_positions (whm_config_T **configs, /* Print all positions of all active companies. */
					 whm_sheet_T **sheets,
					 int max_ind);
int             whm_modify_previous_date(whm_config_T **configs, /* Modify the number of worked hours of a past date. */
					 whm_sheet_T **sheets,
					 whm_time_T *time_o,
					 int max_ind);
int             whm_recalculate_year (whm_config_T **configs,    /* Recalculate each sheets of a single company for a given year. */
				      whm_time_T *time_o,
				      int max_ind,
				      char *name,
				      char *year);
int             whm_recalculate_year_all (whm_config_T **configs,/* Recalculate sheets of all active companies for a given year. */
					  whm_time_T *time_o,
					  int max_ind);

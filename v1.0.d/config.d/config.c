...
int whm_backup_file(const char *pathname, char *npathname)
{
  ...
  for (i = 0; i < pathname_len; i++){
    if (i == slash_ind+1) new_pathname[np_ind++] = '#';
...
int whm_create_main_config(const char *filename)
{
  ...
  char night_prime[WHM_DEF_STACK_SIZE]; 
  char vacation_pay[WHM_DEF_STACK_SIZE];
...
#define WHM_MEMSET_WAG() do {				\
  ...
  while(1){
    ...
    memset(vacation_pay, '\0', WHM_DEF_STACK_SIZE);
    printf("\nObtenez-vous votre 4%% de vacance a chaque paie? [Oui/Non]: ");
    if(fgets(vacation_pay, WHM_DEF_STACK_SIZE, stdin) == NULL){
      WHM_ERRMESG("Fgets");
      goto errjmp;
    }
    if (whm_check_input_done(vacation_pay))break;
    WHM_TRIM_TRAILING_NL(vacation_pay);

    memset(night_prime, '\0', WHM_DEF_STACK_SIZE);
    printf("\nY-a-t'il des quarts de nuit?\n(Si applicable)\nA combien s'eleve la prime de nuit? ['-1' indique aucune]: ");
    if (fgets(night_prime, WHM_DEF_STACK_SIZE, stdin) == NULL){
      WHM_ERRMESG("Fgets");
      goto errjmp;
    }
    if (whm_check_input_done(night_prime)) break;
    WHM_TRIM_TRAILING_NL(night_prime);
    for (i = 0; i < WHM_DEF_STACK_SIZE && night_prime[i] != '\0'; i++){
      if (isdigit(night_prime[i]) || night_prime[i] == DOT || night_prime[i] == DASH) ;
      else {
	errno = WHM_INVALIDCHAR;
	goto errjmp;
      }
    }
    ...
    fprintf(filestream, "%s\n%s\n\n",
	    ((vacation_pay[0] == 'O' || vacation_pay[0] == 'o') ? "1" : "0"),
	    night_prime);
			...
			int *numof_positions)
{
  ...
  /*
   * Night prime has been pushed last on the stack,
   * preceded by vacation pay, wages, names of position, the numbers of said positions,
   ...
   */
  job_array[*ja_ind]->night_bonus  = atof(whm_pop(mc_stack));
  job_array[*ja_ind]->vacation_pay = atoi(whm_pop(mc_stack));
  for (i = *numof_positions-1; i >= 0; i--){
...
whm_job_info_T** whm_read_main_config(const char *filename, size_t *ja_ind)
{
  ...
  int            numof_positions = 0, numof_entries = 0;
...
#define WHM_PUSH_N_CLEAN(stack, value, val_ind, value_len) do {		\
  ...
  for (content_ind = 0;
	 ...
	 * nl_count < 6 cause line 6 is a double integer indicating the night bonus.
	 */
	if (isdigit(temp[0]) && mc_stack->tos > 0 && nl_count < 6) numof_positions = atoi(temp);
  ...
  /* 
   * If there was a main configuration file but no entry was present
   * (ex: a problem happened during it's creation), we go back to the call 
   * to whm_create_main_config().
   */
  if (job_array[0]->numof_positions <= 0) {
    if (remove(filename) != 0){
      WHM_ERRMESG("Remove");
      goto errjmp;
    }
    ...
    numof_positions = 0;
    numof_entries = 0;
    goto create_main_config;
  }
  
  /* Cleanup behind yourself! */
  if (main_config_stream){
    fclose(main_config_stream);
    main_config_stream = NULL;
  }
			  ...
			  size_t *ja_ind)
{
  ...
  while (i < *ja_ind){
    ...
    fprintf(filestream, "\n%d\n%f\n\n", job_array[i]->vacation_pay,
	    job_array[i]->night_bonus);
    ++i;
  }
...
/* 
 * Handles all options passed on the command
 * line and their arguments.
		       ...
		       whm_time_T *time_o)
{
  ...
  if (!argv || !sheet || !job_info || !time_o){
    errno = EINVAL;
    return -1;
  }
  errno = 0;
  while ((opt = getopt(argc, argv, "pr:auN:P:W:D:M:Y:")) != -1){
    switch (opt){
    case 'p':

...
int main(int argc, char **argv)
{
  ...
  for(; week_ind < 6; week_ind++)
    if (atoi(time_o->week) == read_sheets[0]->week[week_ind]->week_number) break;
  if (week_ind >= 6){
    errno = WHM_INVALIDWEEKIND;
    goto errjmp;
  }
  for (; day_ind < 7; day_ind++)
    if(strcmp(time_o->day, WHM_EN_WEEK_DAYS[day_ind]) == 0) break;
  ...
  for (sht_ind = 0; sht_ind < ja_ind; sht_ind++){
     ...
     */
    for(pos_ind = 0; pos_ind < job_array[sht_ind]->numof_positions; pos_ind++)
      if (read_sheets[sht_ind]->week[week_ind]->daily_info[day_ind]->pos_hours[pos_ind] == -1)
	is_filled = -1;
    if (is_filled == -1)
      if (whm_get_user_hours(read_sheets[sht_ind],
			     job_array[sht_ind],
			     time_o,
			     week_ind, day_ind) != 0){
	WHM_ERRMESG("Whm_get_user_hours");
	goto errjmp;
      }

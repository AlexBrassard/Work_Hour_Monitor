			...
			whm_time_T *time_o)
{
  ...
  /* The amount of hour sheets to read is gotten from the main configuration file. */
  while (sheet_ind < numof_sheets){
    ...
    fclose(sheet_stream);
    sheet_stream = NULL;
    /* Reset some variables before each itterations. */
    tmp_char = 0;
    sht_char = 0;
    ...
    */
	...
	while(sheet_content[sht_char] != '\0' && sheet_content[sht_char] != SPACE) sht_char++;
	continue;
      }
      temp_string[tmp_char++] = sheet_content[sht_char++];
    }
...
done  This function should update only those hour sheets that havn't been
      updated yet and silently skip the ones that have been updated.
 This function should check every days of the hour sheet and
 recalculate totals everytime, in case a user edit an entry manualy.
			  ...
			  whm_time_T *time_o)
{
  ...
  if (interactive){
    ...
    while (pos_ind < job_info->numof_positions){
      if (hour_sheet->week[week_ind]->daily_info[week_day]->pos_hours[pos_ind] != -1) {
	pos_ind++;
	continue;
      }
  ...
  for (i = 0; i < job_info->numof_positions; i++)
    earnings[i] = (atof(job_info->wage[i])) *

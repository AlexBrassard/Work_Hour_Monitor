...
#define WHM_MEMSET_WAG() do {				\
  ...
  while(1){
    ...
    printf("Obtenez-vous votre 4%% de vacance a chaque paie? [Oui/Non]: ");
			...
			int *numof_positions)
{
  ...
  /*
   * Vacation pay has been pushed last on the stack,
   * preceded by wages, names of position, the numbers of said positions,
		       ...
		       whm_time_T *time_o)
{
  ...
  /* 
   * An option must begin with a single or double dash.
   * A single dash option must be followed by exactly 1 
   * alphanumeric character, the option name.
   * A double dash option must be followed by 1 or more alphanumeric 
   * character (up to <<constant name goes here>>), the option name.
   * Any argument not begining with dash(es) are considered arguments to the 
   * previous (- --) option. If there's no previous option, the arguments are discarded.
   */
  return 0;
} /* whm_handle_options() */
